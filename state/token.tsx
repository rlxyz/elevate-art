...
function useToken() {
  ...
  const getPublicMaxInvocation = async (address: string): Promise<number> => {
    ...
    const totalMintLeft = COLLECTION_DISTRIBUTION.maxPublicBatchPerAddress - mintCount
  ...
  const syncStatus = async (): Promise<void> => {
    ...
    try {
      if (address) {
        ...
        if (_presaleTime == null || _publicTime == null || _claimTime === null) {
          setMintPhase('none')
        }
...
interface TokenProps {
  ...
  mintPhase: MintPhase
  claimMint: (invocations: number, callbackFn?: (txHash: string) => void) => Promise<void>
  presaleMint: (
    invocations: number,
    callbackFn?: (txHash: string) => void,
  ) => Promise<void>
