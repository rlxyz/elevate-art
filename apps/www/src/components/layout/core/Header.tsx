    ...
    <div className="flex flex-row items-center justify-center space-x-3">
      <aside className="flex flex-row items-center justify-center space-x-3">
        {externalRoutes.map((item) => {
          return (
            <LinkComponent key={item.name} href={item.href} rel="noreferrer nofollow" target="_blank">
              <span className="hover:text-black text-darkGrey cursor-pointer text-xs" aria-hidden="true">
                {item.name}
              </span>
            </LinkComponent>
          );
        ...
        {socialRoutes.map((item) => (
          <LinkComponent key={item.name} href={item.href} rel="noreferrer nofollow" target="_blank">
            <item.icon className="hover:text-black text-darkGrey h-4 w-4 cursor-pointer" aria-hidden="true" />
          </LinkComponent>
        ))}
      </aside>
      {authenticated ? (
        ...
        <NextLink href="/connect">
          <span className="bg-black text-white h-fit w-fit cursor-pointer rounded-[5px] p-2 text-xs">Connect</span>
        </NextLink>
      )}
    </div>
  );
                        ...
                        <div className="bg-white relative space-y-1 p-2">
                          {organisations.length > 0 ? (
                            <div className="space-y-1">
                              <span className="text-darkGrey text-xs">Your Teams</span>
                              <div>
    ...
    <aside className="-ml-5">
      <ul className="flex list-none">
        {links.map(({ name, enabled, href, loading }, index: number) => {
          return (
            <li key={index} className={enabled ? "space-between relative flex items-center" : ""}>
              <div className={clsx("mb-1", loading && "pointer-events-none")}>
                <Link disabled={loading} enabled={enabled} title={capitalize(name)} href={href} />
              </div>
              {enabled && <motion.div className="bg-black absolute bottom-[-1px] left-0 right-0 mx-3 h-[2px]" layoutId="underline" />}
            </li>
          );
        })}
      </ul>
