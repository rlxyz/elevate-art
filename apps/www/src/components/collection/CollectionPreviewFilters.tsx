  ...
  return (
    ...
    >
      {({ handleChange, submitForm }) => (
        <Form>
          <div className="max-h-[70vh] overflow-y-scroll rounded-[5px] no-scrollbar">
            {layers?.map((layer: LayerElement & { traitElements: TraitElement[] }, optionIdx: number) => (
              <div key={layer.id} className={clsx("flex flex-col text-xs", layer.traitElements.length === 0 && "cursor-not-allowed")}>
                <div
                  ...
                  className={`py-3 text-xs hover:bg-accents_8 ${layerDropdown === optionIdx ? "font-semibold" : ""}`}
                >
                  <div className="flex justify-between px-3">
                    <label>{truncate(layer.name)}</label>
                    <div className="flex items-center space-x-2">
                      <span className="text-xs">{layer.traitElements.length}</span>
                      {layerDropdown !== optionIdx ? <ChevronDownIcon className="h-3 w-3" /> : <ChevronUpIcon className="h-3 w-3" />}
                    </div>
                  </div>
                </div>
                <div
                  className={
                    layerDropdown === optionIdx ? "border-mediumGrey max-h-[17.5rem] overflow-y-scroll border-b no-scrollbar" : "hidden"
                  }
                >
                  {layer.traitElements
                    ...
                    .map((traitElement: TraitElement) => {
                      return (
                        <div
                          className="hover:bg-lightGray flex w-full flex-row items-center justify-between py-3 px-3"
                          key={traitElement.id}
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Field
                              ...
                              className="border-mediumGrey bg-hue-light h-4 w-4 rounded-[3px] border"
                              onChange={(e: any) => {
                                handleChange(e);
                                submitForm();
                              }}
...
export const FilterByRarity = () => {
  ...
  return (
    ...
    >
      {({ handleChange, submitForm }) => (
        <Form>
          <div className={clsx("max-h-[calc(100vh-17.5rem)] overflow-y-scroll rounded-[5px] no-scrollbar")}>
            {filters.map(({ value }, optionIdx: number) => (
              <div key={optionIdx} className="flex flex-col text-xs">
                <div className={`py-3 text-xs hover:bg-accents_8`}>
                  <div className="flex justify-between px-3">
                    <label>{value}</label>
                    <div className="flex items-center space-x-2">
                      <span className="text-xs">
                        <Field
                          ...
                          name="checked"
                          value={value}
                          className="bg-hue-light h-4 w-4 rounded-[3px] border border-border"
                          onChange={(e: any) => {
                            handleChange(e);
                            submitForm();
                          }}
...
const Index = () => {
  ...
  return (
    <>
      <div
        className={clsx(
          isLoading ? "h-full animate-pulse rounded-[5px] bg-accents_7 bg-opacity-50" : "border border-border",
          "space-y-1 rounded-[5px]",
        )}
      >
      ...
      <div
        className={clsx(
          isLoading ? "h-[32rem] animate-pulse rounded-[5px] bg-accents_7 bg-opacity-50" : "border border-border",
          "space-y-1 rounded-[5px]",
        )}
      >
        <div className={clsx(isLoading && "invisible")}>
