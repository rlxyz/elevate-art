  ...
  return (
    <div className={clsx(className, "grid grid-cols-5 gap-3")}>
      {table
        ...
        .map(({ original }, index) => {
          return (
            <div key={original.id} className="border-mediumGrey h-auto w-auto flex-col rounded-[5px] border">
              <div className="border-mediumGrey flex h-[10rem] w-auto items-center overflow-hidden border-b lg:h-[12.5rem] xl:h-[15rem] 2xl:h-[20rem]">
                {!original.readonly && (
                  <img loading="lazy" className="border-mediumGrey h-auto w-auto border-t border-b" src={original.imageUrl} />
                )}
              </div>
              <div className="flex flex-col space-y-1 px-1 py-2">
                <span className="w-full overflow-hidden text-xs font-semibold">{truncate(original.name)}</span>
                <div className="flex flex-col text-[0.6rem]">
                  <span className="w-full overflow-hidden font-semibold">{Big(original.weight).toFixed(4)}%</span>
                  <span className="text-darkGrey w-full overflow-hidden">Update {timeAgo(original.updatedAt)}</span>
                </div>
              </div>
            </div>
          );
