  ...
  return (
    <Disclosure {...props}>
      {({ open }) => (
        <>
          <Disclosure.Button
            className={`border-mediumGrey w-full space-y-2 rounded-[5px] border p-2`}
          >
            <div className="grid grid-cols-10 ">
                ...
                <div className="flex space-x-3">
                  ...
                  <div className="flex w-full flex-col items-start space-y-1">
                    ...
                    <span className="text-darkGrey text-xs">
                      {formatBytes(traits.reduce((a, b) => a + b.size, 0))}
                    </span>
                  </div>
                </div>
              </div>
              <div className="col-span-1 flex h-full flex-col items-end justify-center">
                {open ? (
                  <ChevronUpIcon className="row-span-1 h-3 w-3" />
                ) : (
                  <ChevronDownIcon className="row-span-1 h-3 w-3" />
                )}
                <span className="text-xs">
                  {traits.filter((file) => file.uploaded).length} /{" "}
                  {traits.length}
                </span>
              </div>
            </div>
            <div className="bg-lightGray border-mediumGrey flex h-2 w-full items-start rounded-[5px] border text-left">
              <motion.div
                style={{
                  width: `${
                    (traits.filter((x) => x.uploaded).length / traits.length) *
                    100
                  }%`,
                }}
            ...
            <Disclosure.Panel>
              <div
                className={clsx(
                  ...
                  gridSize === "md" && "grid-cols-6",
                  gridSize == "lg" && "grid-cols-12",
                )}
              >
                {traits
                  .sort((a) =>
                    ...
                    return (
                      ...
                      >
                        <img
                          ...
                          height={200}
                          src={item.imageUrl}
                          className="border-mediumGrey rounded-[5px] border"
                        />
                        {item.uploaded && (
                          <CheckCircleIcon className="text-greenDot absolute top-0 right-0 m-1 h-3 w-3 rounded-[3px]" />
                        )}
                        {withTooltip &&
                          (item.type === "existing" ? (
                            <Tooltip
                              ...
                              description={
                                "Duplicate file found. We are reuploading the image."
                              }
                            />
                          ) : item.type === "new" ? (
                            <Tooltip
                            ...
                            <Tooltip
                              ...
                              description={
                                "This file cant be upload. Try again..."
                              }
                            />
                          ) : null)}
