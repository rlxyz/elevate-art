    ...
    <div className="flex flex-row items-center justify-center space-x-3">
      <aside className="flex flex-row items-center justify-center space-x-3">
        {externalRoutes.map((item) => {
          return (
            ...
            >
              <span
                className="hover:text-black text-darkGrey cursor-pointer text-xs"
                aria-hidden="true"
              >
                {item.name}
              </span>
        ...
        {socialRoutes.map((item) => (
          ...
          >
            <item.icon
              className="hover:text-black text-darkGrey h-4 w-4 cursor-pointer"
              aria-hidden="true"
            />
          </LinkComponent>
        ))}
      ...
      {authenticated ? (
        ...
        <NextLink href="/connect">
          <span className="bg-black text-white h-fit w-fit cursor-pointer rounded-[5px] p-2 text-xs">
            Connect
          </span>
        </NextLink>
      )}
...
export const HeaderInternalAppRoutes = ({
  routes,
}: HeaderInternalAppRoutesProps) => {
          ...
          <div key={index} className="flex items-center justify-center">
            ...
            <Popover className="flex space-x-1">
              <NextLinkComponent href={href} className="w-fit">
                {isLoading ? (
                  <div className="bg-mediumGrey h-5 w-36 animate-pulse rounded-[5px]" />
                ) : (
                  <>
                    <div
                      className={clsx(
                        organisations ? "text-black" : "text-darkGrey",
                        "py-1",
                      )}
                    >
                      {current === OrganisationNavigationEnum.enum.You ||
              ...
              {organisations ? (
                <>
                  <Popover.Button className="group inline-flex items-center rounded-[5px] text-xs">
                    <SelectorIcon className="text-black h-4 w-4" />
                  </Popover.Button>
                  <Transition
                    as={Fragment}
                  ...
                  >
                    <Popover.Panel className="absolute z-10 w-screen max-w-xs py-6">
                      <div className="ring-black overflow-hidden rounded-[5px] shadow-lg ring-1 ring-opacity-5">
                        <div className="bg-lightGray border-mediumGrey border-b py-2">
                          <div className="relative rounded-[5px]">
                            <Link
                              external
                              href={`/${OrganisationNavigationEnum.enum.Dashboard}`}
                            >
                              <div className="text-darkGrey hover:text-black flex w-full flex-row items-center justify-between py-2 pl-2 pr-4">
                                ...
                                {current ===
                                OrganisationNavigationEnum.enum.Dashboard ? (
                        ...
                        <div className="bg-white relative space-y-1 p-2">
                              ...
                              <div>
                                {organisations.map(({ name, type, id }) => (
                                  ...
                                  >
                                    ...
                                    >
                                      <div className="flex items-center space-x-2">
                                        <div className="bg-blueHighlight h-5 w-5 rounded-full" />
                                        <span>
                                          {type ===
                                          OrganisationDatabaseEnum.enum.Team
                                            ? name
                                            : capitalize(
                                                OrganisationNavigationEnum.enum
                                                  .You,
                                              )}
                                        </span>
                                      </div>
...
export const HeaderInternalPageRoutes = ({
  links,
}: HeaderInternalPageRoutesProps) => {
  return (
        ...
        {links.map(({ name, enabled, href, loading }, index: number) => {
          return (
            <li
              key={index}
              className={
                enabled ? "space-between relative flex items-center" : ""
              }
            >
              ...
              {enabled && (
                <motion.div
                  className="bg-black absolute bottom-[-1px] left-0 right-0 mx-3 h-[2px]"
                  layoutId="underline"
                />
              )}
            </li>
