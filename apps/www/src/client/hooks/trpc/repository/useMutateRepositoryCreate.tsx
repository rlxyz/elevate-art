...
export const useMutateRepositoryCreate = ({
  setRepository,
}: {
  ...
  const { mutateAsync: createRepository } =
    trpc.repository.create.useMutation();
  ...
  const mutate = async ({
    ...
    if (isLoading || !organisation) {
      ...
      notifyError(
        "We couldnt find your team. Please refresh the page to try again.",
      );
      return;
    }

    const repositoryName: string =
      (files[0]?.path?.split("/")[1] as string) || "";
    const layers = getRepositoryUploadLayerObjectUrls(files);

    try {
      setUploadedFiles(layers);
      notifySuccess(
        "Upload format is correct. We are creating the project for you.",
      );
      ...
      notifySuccess(
        "Project created. We are uploading the images now. Do not leave this page!",
      );
      ...
      const filePromises = files.map(async (file) => {
        return new Promise((resolve, reject) => {
          ...
          const traitElement = response.layers
            .find((x) => x.name === layerName)
            ?.traitElements.find((x) => x.name === traitName);
          ...
          reader.onload = async () => {
            try {
              ...
              setUploadedFiles((state) =>
                produce(state, (draft) => {
                  const trait = draft[layerName]?.find(
                    (x) => x.name === traitName,
                  );
                  if (!trait) return;
                  trait.uploaded = true;
