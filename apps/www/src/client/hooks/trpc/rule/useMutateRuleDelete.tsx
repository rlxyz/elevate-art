      ...
      ctx.layerElement.findAll.setData({ repositoryId }, (old) => {
        ...
        const next = produce(old, (draft) => {
          ...
          const primary = allTraitElements.find(
            (l) => l.id === data.primaryTraitElementId,
          );
          const secondary = allTraitElements.find(
            (l) => l.id === data.secondaryTraitElementId,
          );
          if (
          ...
          const ruleIndexPrimary = primary.rulesPrimary.findIndex(
            (r) => r.id === data.id,
          );
          const ruleIndexSecondary = secondary.rulesSecondary.findIndex(
            (r) => r.id === data.id,
          );
          ...
          notifySuccess(
            `Deleted ${primary.name} ${data.condition} ${secondary.name} rule`,
          );
        });
        return next;
      });
