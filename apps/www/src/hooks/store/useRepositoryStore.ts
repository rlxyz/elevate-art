...
interface RepositoryStoreFunctionInterface {
  ...
  setRarityFilter: (
    filter: "Top 10" | "Middle 10" | "Bottom 10" | "All",
  ) => void;
  setTokenRanking: (indices: { score: number; index: number }[]) => void;
  setTraitFilters: (
    filters: { layer: LayerElement; trait: TraitElement }[],
  ) => void;
...
interface RepositoryStoreInterface
  extends RepositoryStoreFunctionInterface,
    RepositoryStoreStateInterface {}
...
export const createRepositoryStore = create<RepositoryStoreInterface>()(
  persist(
    (set) => ({
      ...initialState,
      setTraitFilteredTokens: (tokens: number[]) =>
        set((_) => ({ traitFilteredTokens: tokens })),
      setRarityFilter: (filter: "Top 10" | "Middle 10" | "Bottom 10" | "All") =>
        set((_) => ({ rarityFilter: filter })),
      setTokens: (tokens: number[]) => set((_) => ({ tokens: tokens })),
      setTraitFilters: (
        filters: { layer: LayerElement; trait: TraitElement }[],
      ) => set((_) => ({ traitFilters: [...filters] })),
      setTraitMapping: ({
        tokenIdMap,
        traitMap,
      }: {
      ...
      setTokenRanking: (indices: { score: number; index: number }[]) =>
        set((_) => ({ tokenRanking: indices })),
