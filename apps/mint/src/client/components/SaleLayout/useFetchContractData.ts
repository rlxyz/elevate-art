...
export const useFetchContractUserData = ({ contractAddress, userAdress, chainId, enabled = true, version }: UseContractUserRead) => {
  ...
  return useContractReads({
    ...
    contracts: [
      { address: contractAddress, abi: RhapsodyContractConfig.contractInterface, functionName: 'mintOf', args: [userAdress], chainId },
    ],
    watch: true,
    cacheTime: 2_000,
    staleTime: 2_000,
...
export const useFetchContractData = ({ contractAddress, chainId, enabled = true, version }: UseContractRead) => {
  ...
  return useContractReads({
    scopeKey: `erc721:${version}:${chainId}:${contractAddress}`,
    contracts: [
      { address: contractAddress, abi: RhapsodyContractConfig.contractInterface, functionName: 'totalSupply', chainId },
      { address: contractAddress, abi: RhapsodyContractConfig.contractInterface, functionName: 'maxPublicBatchPerAddress', chainId },
      { address: contractAddress, abi: RhapsodyContractConfig.contractInterface, functionName: 'collectionSize', chainId },
    ],
    watch: true,
    cacheTime: 2_000,
    staleTime: 2_000,
