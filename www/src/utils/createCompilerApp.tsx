...
export const createCollectionSeed = (collectionId: string, generation: number) => {
  return parseInt(
    ethers.utils.keccak256(ethers.utils.toUtf8Bytes(`${collectionId}-${generation}`)).toString(),
    16
  )
}
...
export const createCompilerApp = (repositoryName: string): App => {
  return new App({
    configs: repositoryName === 'roboghosts' ? roboghostLayerConfig : reflectionsLayerConfig,
    imageFormat: {
      width: 300,
      height: 300,
      smoothing: false,
...
const reflectionsLayerConfig = [
  ...
  // },
  {
    ...
    options: {
      ...
      exclude: {
        LIGHTS: ['SPRING', 'ICELAND', 'SNOWY VILLAGE', 'WINTER', 'VENICE', 'DEAD TREES'],
      },
      occuranceRate: 1 / 8,
    },
  },
