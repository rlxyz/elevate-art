...
const Home: NextPage = () => {
  const { data: organisation }: { data: Organisation | null | undefined } = trpc.useQuery([
    'organisation.getOrganisationByName',
    { name: 'sekured' },
  ])

  const { data: repository } = trpc.useQuery([
    ...
    <>
      ...
      <main className='container mx-auto flex flex-col items-center justify-center min-h-screen p-4'>
        ...
        <div className='pt-6 text-2xl text-blue-500 flex justify-center items-center w-full flex-col'>
          {organisation ? <p>{organisation?.id}</p> : <p>Loading..</p>}
          {repository ? <p>{repository?.id}</p> : <p>Loading..</p>}
        </div>
      </main>
    </>
  )
