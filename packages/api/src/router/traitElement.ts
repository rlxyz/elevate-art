...
const TraitElementDeleteInput = z.array(
  z.object({
    id: z.string(),
...
const TraitElementUpdateNameInput = z.array(
  z.object({
    name: z.string(),
...
export const traitElementRouter = router({
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const response: Result<DeleteTraitElementResponse[]> =
        await deleteImageFilesFromCloudinary(
          traitElements.map((x) => ({
            r: x.repositoryId,
            l: x.layerElementId,
            t: x.id,
  ...
  createManyByLayerElementId: protectedProcedure
    .input(TraitElementCreateManyByLayerElementIdInput)
    .mutation(async ({ ctx, input }) => {
  ...
  updateWeight: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      /** Run sequential operations; as they don't depend on each other. */
      return await updateManyByField(
        ctx.prisma,
        "TraitElement",
        "weight",
        traitElements,
